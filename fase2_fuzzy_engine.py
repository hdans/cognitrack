...
def build_fuzzy_variables():
    ...
    emosi["Negatif"]  = fuzz.trimf(emosi.universe, [0.0, 0.0, 1.5])
    emosi["Bingung"]  = fuzz.trimf(emosi.universe, [1.5, 3.0, 4.5])
    emosi["Netral"]   = fuzz.trimf(emosi.universe, [3.5, 5.0, 6.5])
    emosi["Positif"]  = fuzz.trimf(emosi.universe, [7.5, 10.0, 10.0])
...
def build_rules(ear, pose, emosi, engagement):
    ...
    rule3 = ctrl.Rule(
        antecedent=(ear["Waspada"] & pose["Fokus"] & emosi["Bingung"]),
        consequent=engagement["Sedang"],
        label="R3: Waspada+Fokus+Bingung → Sedang (perhatian tapi bingung)"
    )
    rule4 = ctrl.Rule(
...
def plot_membership_functions(ear, pose, emosi, engagement):
    ...
    COLOR_MAP = {
        ...
        "Negatif": "#e74c3c",   "Bingung": "#e67e22",
        "Netral": "#f1c40f",    "Positif": "#2ecc71",
...
def run_simulation_demo(simulation):
    ...
    scenarios = [
        ...
        {
            "nama":    "Siswa Bingung tapi Perhatian",
            "ear":     0.32,    # Waspada
            "pose":    8.0,     # Fokus
            "emosi":   "Bingung",
    ...
    print("\n✓ Semua skenario berhasil dikomputasi.")
    return results
